import React, { useState, useEffect } from "react";
import { useDispatch, useSelector } from "react-redux";
import {
  fetchApplicantsForJob,
  updateApplicantStatus,
  fetchEmployerJobs,
} from "../../features/employer/employerJobSlice.js";

const ApplicationsPage = () => {
  const dispatch = useDispatch();
  const { jobs, selectedJob, applicants, loading, error } = useSelector(
    (state) => state.employerJobs
  );

  const [showApplicants, setShowApplicants] = useState(false);
  const [openApplicant, setOpenApplicant] = useState(null);

  // Fetch employer's jobs first
  useEffect(() => {
    dispatch(fetchEmployerJobs());
  }, [dispatch]);

  // Pick the latest job by date
  const latestJob =
    jobs && jobs.length > 0
      ? [...jobs].sort(
        (a, b) => new Date(b.date_posted) - new Date(a.date_posted)
      )[0]
      : null;

  // Fetch applicants when "View Applicants" is clicked
  const handleViewApplicants = () => {
    if (!latestJob) return;
    if (!showApplicants) {
      dispatch(fetchApplicantsForJob(latestJob._id));
    }
    setShowApplicants((prev) => !prev);
  };

  // Handle approve/decline
  const handleUpdateStatus = (seekerId, status) => {
    if (!latestJob) return;
    dispatch(updateApplicantStatus({ jobId: latestJob._id, seekerId, status }));
  };

  if (loading && !jobs.length) {
    return (
      <div className="flex justify-center items-center min-h-screen text-white">
        Loading...
      </div>
    );
  }

  if (error) {
    return (
      <div className="flex justify-center items-center min-h-screen text-red-500">
        {error.message || "Something went wrong"}
      </div>
    );
  }

  if (!latestJob) {
    return (
      <div className="flex justify-center items-center min-h-screen text-gray-300">
        No jobs found.
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center p-6">
      {/* Job Card */}
      <div className="bg-gray-800 rounded-lg shadow-md w-full max-w-2xl p-6 border border-gray-700">
        <h2 className="text-xl font-bold">{latestJob.title}</h2>
        <p className="text-gray-400">{latestJob.location}</p>
        <p className="mt-1 text-sm">Salary: {latestJob.salary}</p>
        <p className="mt-1 text-sm">Type: {latestJob.jobType}</p>

        <button
          onClick={handleViewApplicants}
          className="mt-4 px-4 py-2 bg-[#B85042] hover:bg-red-700 rounded-lg"
        >
          {showApplicants ? "Hide Applicants" : "View Applicants"}
        </button>

        {/* Applicants Dropdown */}
        {showApplicants && (
          <div className="mt-4 bg-gray-700 rounded-lg p-4 space-y-3">
            {loading && <p className="text-gray-400">Loading applicants...</p>}

            {!loading && (!applicants || applicants.length === 0) && (
              <p className="text-gray-400">No applicants yet.</p>
            )}

            {(applicants || []).map((applicant, idx) => (
              <div
                key={idx}
                className="border border-gray-600 rounded-lg p-3"
              >
                {/* Applicant summary */}
                <div
                  className="flex justify-between items-center cursor-pointer"
                  onClick={() =>
                    setOpenApplicant(
                      openApplicant === idx ? null : idx
                    )
                  }
                >
                  <span className="font-semibold">
                    {applicant.seekerId?.userId?.name || "Unknown"}
                  </span>
                  <span className="text-sm text-gray-400">
                    {openApplicant === idx ? "▲" : "▼"}
                  </span>
                </div>

                {/* Applicant details */}
                {openApplicant === idx && (
                  <div className="mt-3 text-sm space-y-2">
                    <p>
                      <span className="font-medium">Email:</span>{" "}
                      {applicant.seekerId?.userId?.email || "N/A"}
                    </p>
                    <p>
                      <span className="font-medium">Phone:</span>{" "}
                      {applicant.seekerId?.userId?.phone || "N/A"}
                    </p>
                    <p>
                      <span className="font-medium">Experience (years):</span>{" "}
                      {applicant.seekerId?.experienceYears ?? "N/A"}
                    </p>
                    <p>
                      <span className="font-medium">Skills:</span>{" "}
                      {(applicant.seekerId?.skills || []).join(", ")}
                    </p>
                    <p>
                      <span className="font-medium">Education:</span>{" "}
                      {applicant.seekerId?.education?.map((edu, i) => (
                        <span key={i}>
                          {edu.degree} ({edu.startYear}-{edu.endYear}){" "}
                        </span>
                      )) || "N/A"}
                    </p>

                    {/* Resume */}
                    {applicant.seekerId?.resumeUrl && (
                      <p>
                        <a
                          href={applicant.seekerId.resumeUrl}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-blue-400 underline"
                        >
                          View Resume
                        </a>
                      </p>
                    )}

                    {/* Approve / Decline */}
                    <div className="flex gap-3 mt-3">
                      <button className="px-4 py-2 bg-green-600 hover:bg-green-700 rounded-lg">
                        Approve
                      </button>
                      <button className="px-4 py-2 bg-red-600 hover:bg-red-700 rounded-lg">
                        Decline
                      </button>
                    </div>
                  </div>
                )}
              </div>
            ))}


          </div>
        )}
      </div>
    </div>
  );
};

export default ApplicationsPage;